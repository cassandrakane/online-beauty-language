...
ScatterPlot.prototype.initVis = function() {
    ...
    vis.x = d3.scaleTime()
        .range([0, vis.width])
        .domain(d3.extent(vis.data, function(d) { return d.publishDate; }));

    vis.y = d3.scaleLinear()
        .range([vis.height, 0])
        .domain(d3.extent(vis.data, function(d) { return d.title.length; }));
...
ScatterPlot.prototype.updateVis = function(){
    ...
    circle.enter()
        ...
        .attr("r", 2)
        .attr("cy", function(d) {
            return vis.y(d.title.length);
        })
        .attr("cx", function(d) {
            return vis.x(d.publishDate);
        })
        .attr("stroke", "black")
